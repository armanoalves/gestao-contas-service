...
class PagarContaUseCaseTest {

    @Mock private ContaRepository contaRepository;
    @Mock private ContaMapper contaMapper;

    @InjectMocks private PagarContaUseCase useCase;
    ...
    void devePagarConta() {
        ...
        when(contaMapper.toResponse(any(Conta.class))).thenReturn(
                new ContaResponse(null, null, null, Situacao.PAGO, null, null, null, null, null, null)
        );

        PagarContaRequest request = new PagarContaRequest(LocalDate.now());
        ContaResponse response = useCase.executar(contaId, request);
    ...
    void deveLancarExcecaoQuandoContaNaoExiste() {
        ...
        assertThrows(EntityNotFoundException.class,
                () -> useCase.executar(contaId, new PagarContaRequest(LocalDate.now())));
    }
}
